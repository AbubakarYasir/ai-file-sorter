...
constexpr const char* kObservationMissing = "missing";
constexpr const char* kObservationUnknown = "unknown";
...
bool backup_database(sqlite3* source, const std::string& backup_path)
{
    ...
    fs::remove(Utils::utf8_to_path(backup_path), remove_ec);
...
CREATE INDEX IF NOT EXISTS personal_index_path_key_idx
    ON personal_index_entries(path_key);
...
CREATE INDEX IF NOT EXISTS personal_index_scan_root_idx
    ON personal_index_entries(scan_root, observation_state);
)SQL";
...
struct PersonalFileIndex::Impl {
    ...
    bool initialize_schema()
    {
        ...
        if (!had_entries && !had_runs) {
            if (!create_latest_tables(db)) {
                return false;
            }
            return execute(db, "PRAGMA user_version=2;");
        }
        ...
        if (version < kCurrentSchemaVersion ||
            ...
            const std::string backup_path = database_path + ".pre-migration-v" +
                                            std::to_string(version) + ".bak";
            if (!backup_database(db, backup_path)) {
                return false;
            }

            if (!execute(db, "BEGIN IMMEDIATE;")) {
                return false;
            }

            bool ok = create_latest_tables(db);
            if (ok && !column_exists(db, "personal_index_entries", "path_key")) {
                ok = execute(
                    db,
                    "ALTER TABLE personal_index_entries "
                    "ADD COLUMN path_key TEXT NOT NULL DEFAULT ''; ");
            }
            ...
            if (ok && !column_exists(db, "personal_index_entries", "policy_state")) {
                ok = execute(
                    db,
                    "ALTER TABLE personal_index_entries "
                    "ADD COLUMN policy_state TEXT NOT NULL DEFAULT 'included';");
            }
            ...
            if (ok) {
                ok = execute(
                    ...
                    "SET observation_state=CASE WHEN is_present=1 THEN 'present' ELSE 'missing' END, "
                    "policy_state='included';");
            }
            if (ok) {
                ok = backfill_path_keys(db);
            }
            if (ok) {
                ok = execute(
                    db,
                    "CREATE INDEX IF NOT EXISTS personal_index_path_key_idx "
                    "ON personal_index_entries(path_key);"
                    "CREATE INDEX IF NOT EXISTS personal_index_scan_root_state_idx "
                    "ON personal_index_entries(scan_root, observation_state);"
    ...
    bool prepare_root_policy(const std::string& root)
    {
        auto statement = prepare(
            db,
            "UPDATE personal_index_entries SET policy_state='included' WHERE scan_root=?;");
        if (!statement) {
            return false;
        }
    ...
    bool mark_unseen_missing(const std::string& root, std::int64_t run_id)
    {
        auto statement = prepare(
            db,
            "UPDATE personal_index_entries "
            "SET observation_state='missing', is_present=0 "
        ...
        const std::string& policy_state)
    {
        ...
        auto statement = prepare(
            db,
            "UPDATE personal_index_entries "
            "SET observation_state='unknown', policy_state=?, last_seen_run_id=?, "
            "last_indexed_at_ms=? "
...
ON CONFLICT(full_path) DO UPDATE SET
    ...
    StatementPtr prepare_delete_alias()
    {
        return prepare(
            db,
            "DELETE FROM personal_index_entries WHERE path_key=? AND full_path<>?;");
    }
        ...
        std::int64_t run_id)
    {
        ...
        if (sqlite3_step(delete_alias) != SQLITE_DONE) {
            return false;
        }
        ...
        if (metadata.created_at_ms >= 0) {
            sqlite3_bind_int64(statement, 8, metadata.created_at_ms);
        } else {
            sqlite3_bind_null(statement, 8);
        }
        if (metadata.modified_at_ms >= 0) {
            sqlite3_bind_int64(statement, 9, metadata.modified_at_ms);
        } else {
            sqlite3_bind_null(statement, 9);
        }
        if (!metadata.sha256.empty()) {
            bind_text(statement, 10, metadata.sha256);
        } else {
    ...
    const PersonalFileIndexOptions& options)
{
    ...
    for (const auto& root : roots) {
        ...
        try {
            ...
            normalized.push_back(
                RootInfo{path, Utils::path_to_utf8(path), path_identity_key(path)});
        } catch (const std::exception& ex) {
            summary.errors = 1;
            summary.warnings.push_back(
                std::string("Could not normalize scan root: ") + ex.what());
            return summary;
        }
    }
    if (normalized.empty()) {
    ...
    for (const auto& root : normalized) {
        normalized_roots.push_back(root.display);
    }
    ...
    auto persist = [&](const EntryMetadata& metadata,
        ...
        if (!impl_->upsert(
                upsert.get(), delete_alias.get(), metadata, scan_root, summary.run_id)) {
            ++summary.errors;
    ...
    auto mark_skipped_subtree = [&](const EntryMetadata& metadata,
                                    const std::string& scan_root) -> bool {
        if (!begin_transaction()) {
            ++summary.errors;
            return false;
        }
        if (!impl_->mark_descendants_unknown(
                scan_root,
                metadata.path_key,
                summary.run_id,
            ...
            ++summary.errors;
            summary.warnings.push_back(
                "Could not preserve unknown state below skipped path: " + metadata.full_path);
            return false;
        }
        return true;
    };
    ...
    for (const auto& root_info : normalized) {
        ...
        if (!begin_transaction() || !impl_->prepare_root_policy(root_string) ||
            !commit_transaction()) {
            ++summary.errors;
        ...
        if (options.protect_project_directories) {
            if (auto match = impl_->project_detector.detect(root);
                ...
                if (!options.index_protected_project_contents) {
                    ++summary.entries_skipped;
                    if (!mark_skipped_subtree(metadata, root_string) ||
                        !finalize_root_presence(root_string)) {
                        fatal_database_error = true;
                    }
                    continue;
                }
        ...
        while (!pending_directories.empty() && !fatal_database_error) {
            ...
            if (iterator_ec) {
                ...
                ++summary.errors;
                summary.warnings.push_back(
                    "Could not enumerate directory: " + Utils::path_to_utf8(current));
                continue;
            }
            ...
            while (iterator != end && !fatal_database_error) {
                ...
                if (increment_ec) {
                    ...
                    ++summary.errors;
                    summary.warnings.push_back(
                        "Directory enumeration stopped early: " + Utils::path_to_utf8(current));
                }
                ...
                if (is_reparse_or_symlink(entry)) {
                    EntryMetadata metadata = build_metadata(
                        path,
                        is_directory ? kEntryTypeDirectory : kEntryTypeFile,
                        false,
                        std::nullopt,
                        kPolicyReparseSkipped);
                    if (!persist(metadata, root_string) ||
                ...
                if (is_directory && is_windows_system_root(path)) {
                    EntryMetadata metadata = build_metadata(
                        ...
                        kPolicySystem);
                    if (!persist(metadata, root_string) ||
                        !mark_skipped_subtree(metadata, root_string)) {
                        fatal_database_error = true;
                        break;
                    }
                    ++summary.entries_skipped;
                ...
                if (is_directory) {
                    ...
                    if (regular_ec) {
                        ...
                        ++summary.errors;
                        summary.warnings.push_back(
                            "Could not inspect file entry: " + Utils::path_to_utf8(path));
                        if (increment_ec) {
                            break;
                        }
                        continue;
