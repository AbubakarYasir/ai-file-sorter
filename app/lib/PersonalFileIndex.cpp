...
CREATE INDEX IF NOT EXISTS personal_index_scan_root_idx
    ...
    void finish_run(const PersonalFileIndexScanSummary& summary, const char* status) {
        ...
        sqlite3_bind_int64(statement.get(), 1, now_ms());
        sqlite3_bind_text(statement.get(), 2, status, -1, SQLITE_STATIC);
    ...
    bool mark_root_not_present(const std::string& root) {
        auto statement = prepare(
            db,
            "UPDATE personal_index_entries SET is_present=0 WHERE scan_root=?;");
        if (!statement) {
            return false;
        }
...
PersonalFileIndexScanSummary PersonalFileIndex::scan(
    ...
    for (const auto& root : roots) {
        ...
        normalized_roots.push_back(Utils::path_to_utf8(normalized_root(Utils::utf8_to_path(root))));
    }
    ...
    if (!execute(impl_->db, "BEGIN IMMEDIATE;")) {
        ++summary.errors;
        summary.warnings.push_back("Could not begin index transaction");
        impl_->finish_run(summary, "failed");
        return summary;
    }

    for (const auto& root : normalized_roots) {
        if (!impl_->mark_root_not_present(root)) {
            ++summary.errors;
            summary.warnings.push_back("Could not mark previous index rows stale for: " + root);
        }
    }
    if (!execute(impl_->db, "COMMIT;")) {
        ++summary.errors;
        summary.warnings.push_back("Could not commit stale-row update");
    ...
    bool transaction_open = execute(impl_->db, "BEGIN IMMEDIATE;");
    if (!transaction_open) {
        ++summary.errors;
    ...
    auto persist = [&](const EntryMetadata& metadata,
        ...
        ++entries_in_batch;
        if (entries_in_batch >= batch_size && !commit_batch()) {
            ++summary.errors;
    ...
    for (const auto& root_string : normalized_roots) {
        ...
        if (!root_is_directory) {
            ...
            if (fs::is_regular_file(root, regular_ec) && !regular_ec) {
                EntryMetadata metadata = build_metadata(
                    root, kEntryTypeFile, options.compute_sha256, std::nullopt);
        ...
        if (options.protect_project_directories) {
            if (auto match = impl_->project_detector.detect(root);
                ...
                EntryMetadata metadata = build_metadata(
                    root, kEntryTypeProtectedProject, false, match);
                if (!persist(metadata, root_string)) {
                    fatal_database_error = true;
                    break;
                }
        ...
        EntryMetadata root_metadata = build_metadata(
            root, kEntryTypeDirectory, false, std::nullopt);
                ...
                if (is_directory) {
                    ...
                    if (options.protect_project_directories) {
                        if (auto match = impl_->project_detector.detect(path);
                            match && ProtectedProjectDetector::should_skip(*match)) {
                            EntryMetadata metadata = build_metadata(
                                path, kEntryTypeProtectedProject, false, match);
                            if (!persist(metadata, root_string)) {
                                fatal_database_error = true;
                                break;
                            }
                    ...
                    EntryMetadata metadata = build_metadata(
                        path, kEntryTypeDirectory, false, std::nullopt);
                    if (!persist(metadata, root_string)) {
                        fatal_database_error = true;
                        break;
                    }
                    ...
                    EntryMetadata metadata = build_metadata(
                        path, kEntryTypeFile, options.compute_sha256, std::nullopt);
                    if (!persist(metadata, root_string)) {
                        fatal_database_error = true;
                        break;
                    }
    ...
    if (transaction_open) {
        if (fatal_database_error || !execute(impl_->db, "COMMIT;")) {
            execute(impl_->db, "ROLLBACK;");
            transaction_open = false;
            if (!fatal_database_error) {
                ++summary.errors;
                summary.warnings.push_back("Could not commit final index batch");
            }
            fatal_database_error = true;
        } else {
            transaction_open = false;
    ...
    impl_->finish_run(summary, summary.completed ? "completed" : "failed");
    return summary;
}
